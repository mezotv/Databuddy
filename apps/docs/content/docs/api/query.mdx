...
| POST | `/v1/query/compile` | Validate and compile a query without executing it |
...
| `organization_id` | Yes* | Organization ID for organization-scoped query types |
| `timezone` | No | Timezone for date calculations (default: UTC) |

*One of `website_id`, `link_id`, `schedule_id`, or `organization_id` is required unless the signed-in session or global API key supplies an organization context.
