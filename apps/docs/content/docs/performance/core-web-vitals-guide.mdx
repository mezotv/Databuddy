...
description: "Complete guide to Core Web Vitals - LCP, INP, and CLS. Learn how to measure, optimize, and monitor these user-experience signals."
icon: "Zap"
---
...
<CodeBlock 
  language="javascript"
  code={`// Databuddy automatically tracks LCP
...
<CodeBlock 
  language="javascript"
  code={`new PerformanceObserver((entryList) => {
...
<CodeBlock 
  language="html"
  filename="Remove Render-Blocking Resources"
  code={`<!-- async non-critical CSS -->
...
<CodeBlock 
  language="html"
  filename="Critical Rendering Path"
...
### What is INP?
INP measures how quickly a page responds visually to interactions across the visit, such as clicks, taps, and keyboard input.
...
<CodeBlock 
  language="javascript"
  code={`// Databuddy tracks INP when Web Vitals are enabled
  databuddy.track('core_web_vital', {
    metric: 'INP',
    value: inpValue,
    eventType: eventType
...
<Databuddy
  ...
  trackWebVitals // Enables Core Web Vitals tracking
  trackErrors
/>
```
...
- **Blocking JavaScript** - Worsen INP
...
### Issue 2: Third-Party Scripts Hurting INP
...
- **Core Web Vitals scores** - LCP, INP, CLS values
