...
<Callout type="tip" title="Powering the Map View">
  ...
  derived from the user's IP address at the beginning of a session. Page views
  and sessions are tracked automatically; there is no separate SDK option to
  enable them. See [SDK configuration](/docs/sdk/configuration) for the optional
  controls.
</Callout>
...
<Callout type="tip" title="Enabling Performance Data">
  ...
  enable `trackWebVitals={true}` in your [SDK
  configuration](/docs/sdk/configuration). This allows the SDK to collect Core
  Web Vitals from the visitor's browser.
</Callout>
...
<Card title="INP - Interaction to Next Paint">
  **Good**: ≤ 200 milliseconds Measures responsiveness across user
  interactions during the page visit.
</Card>
...
<Card title="Investigations in Slack">
